...
class SpamDetectionEngineTest {
    ...
    /**
     * 표준 룰셋: 각 테스트는 필요한 룰만 인자로 전달한다.
     * 전체 시나리오 테스트에서는 아래 상수를 공유한다.
     */
    ...
    private static final SpamRule PHONE_NUM    = SpamRule.regex  ("PHONE_NUM",    "\\d{3}-\\d{4}-\\d{4}", 0.5);
    ...
    void reasonCodes_distinctPerRule() {
        var rules = List.of(
                SpamRule.keyword("R1", "무료쿠폰", 0.5),
                SpamRule.keyword("R2", "카지노", 0.5)
        );
