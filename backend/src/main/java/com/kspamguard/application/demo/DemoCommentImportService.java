...
public class DemoCommentImportService implements DemoCommentImportUseCase {
    ...
    public DemoCommentImportService(DetectCommentUseCase detectCommentUseCase, List<SpamRule> defaultRules) {
    ...
    public List<DemoDetectionResult> importAndDetect(List<DemoCommentItem> items) {
        return items.stream()
                .map(item -> analyze(item))
                .toList();
    }

    private DemoDetectionResult analyze(DemoCommentItem item) {
        DetectionResult result = detectCommentUseCase.detect(
                new DetectCommentCommand(item.text(), defaultRules)
        );
